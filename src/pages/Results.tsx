...
import { Download, Loader2, Brain, Info, Bot } from 'lucide-react';
import { jsPDF } from 'jspdf';
import { analyzeText, getAnalysisInterpretation } from '../lib/analysis';
import { getAIAnalysis } from '../lib/cloudflare';
...
export function Results() {
  ...
  useEffect(() => {
    const fetchAIAnalysis = async () => {
      try {
        const result = await getAIAnalysis(stories[0].content);
  ...
  const downloadPDF = () => {
    ...
    if (aiAnalysis) {
      ...
      yPos += 10;
      doc.setFontSize(12);
      const aiLines = doc.splitTextToSize(aiAnalysis, 170);
          ...
          {isLoading ? (
            ...
            <div className="prose prose-indigo max-w-none">
              <div className="bg-gray-50 rounded-lg p-4 text-sm text-gray-700">
                <p className="italic mb-2">Note: This analysis is generated by artificial intelligence and should be considered as supplementary insights to the standard analysis above.</p>
              </div>
              <div className="mt-4">
